<template>
	<el-container class="container">
		...
		<el-dialog title="发布" :visible.sync="showPublishDialog" width="45%" :close-on-click-modal="false">
			<el-form label-width="120px">
				<el-form-item prop="title" label="管辖单位">
					<el-select type="text" style="width: 11vw" v-model="publish.police_unit">
						<el-option v-for="option in publishOptions" :key="option.value" :value="option.value" :label="option.label" />
					</el-select>
				</el-form-item>

				<el-form-item prop="title" label="用户">
					<el-input type="text" v-model="publish.user" style="width: 11vw"></el-input>
				</el-form-item>
				<el-form-item prop="endtime" label="有效期">
					<el-date-picker v-model="publish.endtime" value-format="yyyy-MM-dd" style="width: 11vw" type="date" placeholder="请选择日期" />
				</el-form-item>
			</el-form>
			<span slot="footer" class="dialog-footer">
				<el-button @click="showPublishDialog = false">取 消</el-button>
				<el-button type="primary" @click="handlePublishSubmit">确 定</el-button>
			</span>
		</el-dialog>
	</el-container>
</template>
...
import { items, create, update, remove, uploadUrl, download, publish } from '@/api/knowledge'
...
export default {
	...
	data() {
		...
		console.log(mapToArray(MAP.jurisdiction_unit, 'string'))
		return {
			publishOptions: mapToArray(MAP.jurisdiction_unit, 'string'),
	...
	methods: {
		...
		handlePublish(idx, row) {
			this.publish.knowledgeid = row.knowledgeid
