<template>
	<div class="ktv-container">
		...
		<div class="ktv-body" style="padding: 5px">
			...
			<Import @success="getList" path="ktv"></Import>
		...
		<Detail ref="detalDialog" :dialogTittle="dialogTittle" enterprise_id_key="ktvid" :flag="flag" :dialogVisible.sync="dialogVisible" @submit="handleSubmit" :submitDisabled="submitDisabled" :addEditformItems="addEditformItems"></Detail>
	</div>
</template>
...
export default {
	...
	methods: {
		handlePerson(index, row) {
			this.$router.push({ path: '/basicinfo/employees/domestic', query: { enterprise: row.recreation_place_name, enterprise_id: row.ktvid } });
		},
		async getList() {
